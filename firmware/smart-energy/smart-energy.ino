...
void setup() {
    ...
    Serial.printf("[SYS] Backend target: http://%s:%d/api/v1\n", BACKEND_HOST, BACKEND_PORT);
...
void loop() {
    ...
    if (now - lastWifiCheck >= WIFI_CHECK_INTERVAL_MS) {
        ...
        if (current != laptopConnected) {
            ...
            if (laptopConnected) {
                ...
                Serial.println("[WIFI] Client Connected: NO - will retry registration when laptop connects");
            }
        } else {
            Serial.printf("[WIFI] Client Connected: %s (%d station)\n",
                          laptopConnected ? "YES" : "NO", wifi.getConnectedClients());
        }
    }

    // ---------- Register device when laptop is connected ----------
    if (laptopConnected && !deviceRegistered && (now - lastRegisterAttempt >= DEVICE_REGISTER_RETRY_MS)) {
        ...
        if (!deviceRegistered) {
            Serial.println("[SYS] Registration failed - will retry");
        }
    }

    // ---------- Periodic measurement send ----------
    if (lastReading.valid && deviceRegistered && (now - lastSend >= MEASUREMENT_INTERVAL_MS)) {
        lastSend = now;
        Serial.println("\n[SYS] --- Sending measurement ---");
        ...
        Serial.printf("[SYS] Send result: %s\n", ok ? "SUCCESS" : "FAILED (will retry next cycle)");
        Serial.println("[SYS] ----------------------------\n");
    }
}