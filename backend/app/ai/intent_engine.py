...
INTENT_RULES = [
    {
        ...
        "patterns": [
            ...
            r"what.*happening\s*now",
            r"what.*going\s*on\s*(right\s*)?now",
            r"whats?\s*up\s*(with|right|now)",
            r"how\s*much\s*load",
            r"how\s*much\s*am\s*i\s*(drawing|running|consuming)",
            r"what.*electricity\s*(am?\s*i\s*using|now|current)",
            r"power\s*now",
            r"load\s*(now|reading|current)",
            r"drawing\s*(now|how\s*much)",
            r"how\s*much\s*electricity\s*(am?\s*i\s*using|now)",
            r"what\s*(is|was)\s*my\s*power",
            r"what\s*(is|was)\s*my\s*electricity\s*(power|usage|load)",
            r"my\s*power\s*(now|right\s*now|current|reading)",
            r"power\s*(of|for)\s*my\s*(home|house|system)",
            r"my\s*(current\s*)?(electricity|power)\s*(right\s*)?now",
            r"(what|how)\s*(is|about)\s*(the\s*)?power",
        ],
        "period": "NOW",
        "confidence": 0.95,
    },
    {
        ...
        "patterns": [
            ...
            r"(?=.*\belectricity\b)(?=.*\bdid\s*i\s*use\b)(?!.*\b(today|this\s*month|this\s*week|so\s*far|this\s*year|this\s*hour|yesterday)\b)",
        ],
        "period": "NOW",
        "confidence": 0.99,
    },
    {
        ...
        "patterns": [
            ...
            r"voltage\s*(high|low|normal|ok|safe)",
            r"is\s*(my|the)\s*voltage",
            r"voltage\s*(now|right\s*now|currently)",
            r"what\s*(is|was)\s*my\s*voltage",
            r"my\s*voltage\s*(now|level|reading|high|low|normal)",
            r"voltage\s*(of|for)\s*my\s*(home|house|system)",
            r"what\s*voltage\s*(do\s*i\s*have|am\s*i\s*getting|is\s*coming)",
        ],
        "period": "NOW",
        "confidence": 0.95,
    },
    {
        ...
        "patterns": [
            ...
            r"amps?\s*now",
            r"current\s*drawing",
            r"how\s*much\s*current\s*(am?\s*i\s*drawing|now)",
        ],
        "period": "NOW",
        "confidence": 0.95,
    },
    {
        ...
        "patterns": [
            ...
            r"how\s*much\s*electricity\s*(have\s*i\s*)?used",
            r"electricity\s*used\s*today",
        ],
        "period": "TODAY",
        "confidence": 0.93,
    },
    ...
    },
    {
        ...
        "patterns": [
            ...
            r"how\s*much\s*electricity\s*(did\s*i|have\s*i)\s*pay",
        ],
        "period": "TODAY",
        "confidence": 0.92,
    },
    ...
    },
    {
        ...
        "patterns": [
            ...
            r"bill\s*per\s*month",
            r"how\s*much\s*will\s*(i|my)\s*bill\s*be\s*(per|each|every)\s*month",
        ],
        "period": "MONTHLY",
        "confidence": 0.95,
    },
    {
        ...
        "patterns": [
            ...
            r"what\s*about\s*(for\s*)?(2|two)\s*months",
            r"and\s*for\s*(2|two)\s*months",
            r"what\s*about\s*the\s*billing\s*period",
        ],
    ...
    },
    {
        "intent": "NORMAL_USAGE",
        "patterns": [
            r"normal\s*(usage|consumption|energy)",
            r"what.*normal\s*(usage|consumption|energy)",
            r"(normal|average|usual|typical)\s*(pattern|profile)",
            r"is\s*(my|the)\s*(usage|consumption|energy)\s*normal",
            r"(am?\s*i|are\s*we)\s*using\s*(too\s*much|normally|normally\s*much)",
            r"am?\s*i\s*using\s*too\s*much\s*(electricity|power|energy)",
            r"is\s*(my|the)\s*(electricity|power|energy)\s*usage\s*(normal|high|low|ok)",
            r"should\s*i\s*(reduce|worry|be\s*worried|concerned)",
            r"why\s*(is|am\s*i\s*paying)\s*(my|the)\s*bill\s*high",
            r"why\s*(is|does)\s*my\s*bill\s*(high|so\s*high|increased|more)",
            r"is\s*my\s*electricity\s*usage\s*normal",
            r"am\s*i\s*spending\s*too\s*much",
            r"is\s*(that|it|this)\s*(high|low|normal|ok|safe|too\s*much|too\s*high|too\s*low)",
        ],
        "period": "RECENT",
        "confidence": 0.88,
    },
    ...
    },
    {
        ...
        "patterns": [
            ...
            r"are\s*we\s*(connected|live|online)",
            r"system\s*(status|online|connected|working)",
            r"is\s*(the|my)\s*system\s*(online|working|connected|up)",
        ],
        "period": "NOW",
        "confidence": 0.92,
    },
    {
        ...
        "patterns": [
            ...
            r"what\s*happened\s*(just\s*now|recently|last)",
            r"when\s*(was|is)\s*(the\s*)?last\s*(time|update|reading)",
            r"how\s*old\s*(is|was)\s*(the\s*)?(data|reading|measurement)",
        ],
        "period": "NOW",
        "confidence": 0.92,
    },
    {
        ...
        "patterns": [
            r"(schedule|set\s*(up)?|add|create|make)\s*.*\b(turn\s*(on|off)|on|off)\b.*\b(at|for|every|daily|once|weekdays|weekends|tomorrow|tonight|morning|afternoon|evening|night|noon|midnight)\b",
            r"(schedule|set\s*(up)?|add|create)\s*.*(every\s*day|at\s*\d|at\s*(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)|tonight|noon|midnight)",
            r"^schedule\b.*",
            r"turn\s+(the\s+)?(it|bulb|light|fan|ac|tv|pump|appliance|device|this)\s*(on|off)\b.*\b(at|every|daily|tomorrow|once|weekdays|weekends|tonight|morning|afternoon|evening|night|noon|midnight)\b",
            r"turn\s*(on|off)\b.*\b(at|every|daily|tomorrow|once|weekdays|weekends|tonight|morning|afternoon|evening|night|noon|midnight)\b",
            r"(every\s*day|daily).*(turn|switch)\s*(on|off).*(at)",
            r"\b(every|daily|tonight|tomorrow|once|weekdays|weekends)\b.*\b(turn|switch)\b.*\b(on|off)\b",
            r"turn\s*(on|off)\b.*\bevery\s*(day|morning|evening|night|monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b",
            r"(?:can\s+you|please|would\s+you|could\s+you)\s*(make|set|schedule|add|create)\b.*\b(turn|switch)\s*(on|off)\b",
            r"(?=.*\b(at\s+(\d+|\w+)|am\b|pm\b|every\s+day|daily|tonight|tomorrow|once|weekdays?|weekends?|morning|afternoon|evening|night|noon|midnight)\b)(?=.*\bturn\b)(?=.*\b(on|off)\b)",
        ],
        "period": "NOW",
        "confidence": 0.94,
    },
    {
        ...
        "patterns": [
            r"\b(change|update|modify|edit|move|reschedule|shift|push)\b\s*(the\s*)?(schedule|timing|time)",
            r"\b(change|update|modify|edit|move|reschedule|shift|push)\b\s*.*\bschedule\b",
            r"\b(change|update|modify|edit|move)\b.*\b(to|at)\s+(\d+|\w+)\s*(am|pm)?\b",
            r"\b(change|update|modify|edit|move)\b.*\b(turn|switch)\s*(on|off)\b.*\b(at|to)\b",
        ],
        "period": "NOW",
        "confidence": 0.97,
    },
    {
        ...
        "patterns": [
            r"\b(delete|remove|cancel|clear|drop)\b\s*.*\bschedule\b",
            r"\bremove\b\s*(the\s*)?\d.*(on|off)\b",
            r"\bcancel\b\s*(the\s*)?\d.*(schedule|on|off)",
        ],
        "period": "NOW",
        "confidence": 0.97,
    },
    {
        ...
        "patterns": [
            r"\b(enable|activate|turn\s*back\s*on|resume)\b\s*(the\s*)?(schedule|scheduler)",
            r"\b(enable|activate|resume)\b\s*.*\bschedule\b",
            r"\bschedule\b.*\b(back\s*on|on\s*again)\b",
        ],
        "period": "NOW",
        "confidence": 0.97,
    },
    {
        ...
        "patterns": [
            r"\b(disable|pause|turn\s*off|switch\s*off|deactivate|stop|kill|halt)\b\s*(the\s*)?(schedule|scheduler|auto)",
            r"\b(disable|pause|deactivate|stop|kill|halt)\b\s*.*\bschedule\b",
            r"\b(turn|switch)\s*off\b.*\bschedule\b",
            r"\bschedule\b\s*back?\s*off\b",
        ],
        "period": "NOW",
        "confidence": 0.97,
    },
    {
        ...
        "patterns": [
            ...
            r"what\s+is\s+scheduled",
            r"what\s+appliances?\s+(are|is)\s+scheduled",
            r"what\s+schedules?\s+exist",
        ],
        "period": "NOW",
        "confidence": 0.9,
    },
